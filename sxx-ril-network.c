...
void requestSetNetworkSelectionAutomatic(void *data, size_t datalen,
		RIL_Token t)
{
	_requestSetNetworkSelectionAutomaticWCDMA(data, datalen, t);
}
...
void _requestSetNetworkSelectionAutomaticWCDMA(void *data, size_t datalen,
		RIL_Token t)
{
	int err = 0;
...
void setRadioState(RIL_RadioState newState)
{
	...
	if (sState != oldState) {
		 ...
		 */
		if (sState == RADIO_STATE_SIM_READY) { /* Radio is on and the SIM interface is available */
			onSIMReady();
...
static int onNetworkStateChanged(char* s, char* sms_pdu)
{
	...
	if ( strStartsWith(s,"+CREG:") ||  strStartsWith(s,"+CGREG") ) {
		err = at_tok_start (&s);
		at_tok_nextint(&s,&reg_value);
		at_tok_nextint(&s,&reg_value);			
		if (err < 0 || reg_value != creg_value) {
			LOGE("GSM Register Changed, old register=%d, new_register=%d\n",last_reg_value, reg_value);
			last_reg_value = reg_value;
			RIL_onUnsolicitedResponse (
		...
		err = at_tok_start (&s);
		at_tok_nextint(&s,&reg_value);
		if (err < 0 || reg_value != last_reg_value) 
		{        	
			LOGE("EVDO Air port changed, old mode=%d, new_mode=%d\n",last_reg_value, reg_value);
			last_reg_value = reg_value;
			RIL_onUnsolicitedResponse (
					RIL_UNSOL_RESPONSE_VOICE_NETWORK_STATE_CHANGED,
					NULL, 0); 
