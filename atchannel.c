...
static char* processUSSD(char *cur)
{
	if(strStartsWith(cur,"+CUSD:")){
		while (*cur != '\0' ){
			cur++;
		}
		return cur;
...
void printArray(char* prefix,unsigned char* buffer, int len)
{
    int i;
    char p[MAX_AT_RESPONSE*4]={0};
    for(i = 0 ; i < len ; i++){
        char t[3];
...
static const char *readline()
{
    ...
    while (p_eol == NULL) {
        ...
        if (count > 0) {
			...
			p_eol = processUSSD(s_ATBufferCur);
			if(!p_eol)
				p_eol = findNextEOL(s_ATBufferCur);
            p_read += count;
        } else if (count <= 0) {
