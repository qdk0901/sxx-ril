...
static int _start_pppd()
{
	...
	char *ppp_params[] = {
		...
		"crtscts", //3
		"connect",  //4
		"/system/bin/chat -v ABORT 'BUSY' ABORT 'NO CARRIER' ABORT 'NO ANSWER' TIMEOUT 6 '' 'AT' '' 'ATDT*99# CONNECT'", //5
		"debug", //6 
		"nodetach", //7
		"ipcp-accept-local", //8
...
//
void reportFakeContextList(RIL_Token *t)
{
	int n = 1;
...
void requestSetupDataCall(void *data, size_t datalen, RIL_Token t)
{
	...
	char ipAddrStr[PROPERTY_VALUE_MAX];
	char *p = NULL;
	...
	//replace the funcking uniwap with 3gwap
	if (apn && !strcmp(apn, "uniwap")) {
		apn = "uniwap";
	}
	...
	if (apn && strcmp(g_apn, apn) && pppd_connect_status()) {
		...
		stop_pppd();
		sleep(1);
		start_pppd(((const char **)data)[3], ((const char **)data)[4]);

		i = 0;
		do{
			LOGD("waiting for pppd switching %d", i);
	...
	if (pppd_connect_status()) {
		...
		reportFakeContextList(&t);	
	}

	return;

error:
	LOGE("SetupDefaultPDP failed.");
	RIL_onRequestComplete(t, RIL_E_GENERIC_FAILURE, NULL, 0);
}
